...
response questions from old AP CS-A exams, 2004 and later. 
Some are from the exams and some were just created to have 
a similar difficulty. These problems are good practice for 
the logic/loop/array aspects of the exam. See also the Basic 
Recursion section and the official college board CS-A page.
*/

/*SCORESINCREASING
Given an array of scores, return true if each score is equal 
or greater than the one before. The array will be length 2 or 
more. 
...
100 next to each other in the array. The array length will be 
at least 2. 
...
/*SCORESCHUMP
Given an array of scores sorted in increasing order, return 
true if the array contains 3 adjacent scores that differ from 
each other by at most 2, such as with {3, 4, 5} or {3, 5, 5}. 
...
separate helper method <br>int average(int[] scores, int start, 
int end) { which computes the average of the elements between 
...
would compute averages with doubles, but here we use ints so 
...
public int scoresAverage(int[] scores) {
	...
	int half = scLen / 2;
	return Math.max(average(scores, 0, half), average(scores, half, scLen));
}
...
hasOne(220) → false
*/
// BT - I am doing this a slightly different way than they want
public boolean hasOne(int n) {
	String strcheck = "" + n;
	char[] chCheck = strcheck.toCharArray();
	for (int i = 0; i < chCheck.length; i++){
		if (chCheck[i] == '1') {
			return true;
		}
	}
...
*/
// BT - What I did on the above problem, won't work here.
public boolean dividesSelf(int n) {
	int len = String.valueOf(n).length();
	int numCheck = n;
	for (int i = 0; i < len; i++) {
		int numRight = numCheck % 10;
		if (numRight == 0) {
			return false;
		}
		if (n % numRight != 0) {
...
public boolean isEndy(int num) {
	if ((num <= 100 && num >= 90) || (num <= 10 && num >= 0)) {
		return true;
	}
	else {
		return false;
...
other array, the 2nd to the 2nd and so on. Count the number 
of times that the 2 strings are non-empty and start with the 
...
public int matchUp(String[] a, String[] b) {
	...
	for (int i = 0; i < a.length; i++) {
		...
		else if (a[i].substring(0,1).equals(b[i].substring(0,1))) {
			counter++;
		}
	}
	return counter;
...
/*SCOREUP
The "key" array is an array containing the correct answers to 
an exam, like {"a", "a", "b", "b"}. the "answers" array contains 
 student's answers, with "?" representing a question left blank. 
 he two arrays are not empty and are the same length. Return the 
...
currences of the target string, make a new array of the correct 
...
*/
// BT - Could do this with one loop, but I prefer this for readability
public String[] wordsWithout(String[] words, String target) {
	...
	int j = 0;
	String[] targetArray = new String[counter];
	for (int i = 0; i < words.length; i++) {
		if (!words[i].equals(target)) {
			targetArray[j] = words[i];
			j++;
		}
	}
	return targetArray;
...
score is one which is a multiple of 10, such as 40 or 90. Return 
the sum of largest special score in A and the largest special 
score in B. To practice decomposition, write a separate helper 
method which finds the largest special score in an array. Write 
your helper method after your scoresSpecial() method in the 
JavaBat text area. 
...
of 1*2 + 5 = 7. The start end end index will both be valid indexes 
...
indexes into the array, return the number of "big" steps for a 
walk starting at the start index and ending at the end index. 
We'll say that step is big if it is 5 or more up or down. The 
start end end index will both be valid indexes into the array 
with start <= end. 
...
/*USERCOMPARE
We have data for two users, A and B, each with a String name and 
n int id. The goal is to order the users such as for sorting. 
Return -1 if A comes before B, 1 if A comes after B, and 0 if 
they are the same. Order first by the string names, and then by 
the id numbers if the names are the same. Note: with Strings 
str1.compareTo(str2) returns an int value which is 
negative/0/positive to indicate how str1 is ordered to str2 
(the value is not limited to -1/0/1). (On the AP, there would 
be two User objects, but here the code simply takes the two 
